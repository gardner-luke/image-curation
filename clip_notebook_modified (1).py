...
ENDPOINT_NAME = "clip-embedding-unified"  # Single unified endpoint
...
class CLIP_IMAGE_EMBEDDING(mlflow.pyfunc.PythonModel):
    ...
    def predict(self, context, model_input, params=None):
        ...
        return model_input[col_name].apply(lambda x: self._get_image_embedding_bytearray(x))
...
class CLIP_IMAGE_TEXT_EMBEDDING(mlflow.pyfunc.PythonModel):
    ...
    def predict(self, context, model_input, params=None):
        ...
        if params is None:
            raise ValueError("params must be provided with 'input_type' key")
        ...
        if len(model_input.columns) == 1:
            ...
            # If multiple columns, allow params to specify, otherwise use first
            col_name = params.get('column_name', model_input.columns[0])
        ...
        if input_type.lower() == 'image':
            ...
            return model_input[col_name].apply(lambda x: self._get_text_embedding(x))
        else:
...
def wait_for_endpoint_ready(endpoint_name, max_wait_minutes=30):
...
# MAGIC
# MAGIC Here are examples for both single and batch requests:
...
data = {
    "dataframe_records": [
        {"your_column_name": "BASE64_ENCODED_IMAGE_STRING"}
    ],
    "served_model_name": "image-only"
}

response = requests.post(endpoint_url, headers=headers, json=data)
embedding = response.json()['predictions'][0]  # Single embedding vector

📝 Multimodal Text (Single Record):
----------------------------------
data = {
    "dataframe_records": [
        {"your_text_column": "analyze this text"}
    ],
    "served_model_name": "multimodal",
    "params": {
        "input_type": "text",
        "column_name": "your_text_column"  # Optional if single column
    }
}
...
batch_data = {
    "dataframe_records": [
        {"image_col": "BASE64_IMAGE_1"},
        {"image_col": "BASE64_IMAGE_2"},
        {"image_col": "BASE64_IMAGE_3"},
        {"image_col": "BASE64_IMAGE_4"},
        {"image_col": "BASE64_IMAGE_5"}
    ],
    "served_model_name": "multimodal",
    "params": {
        "input_type": "image",
...
embeddings = response.json()['predictions']  # List of 5 embedding vectors
...
batch_data = {
    ...
    "params": {
        "input_type": "text"
        # column_name auto-detected if single column
    }
}

response = requests.post(endpoint_url, headers=headers, json=batch_data)
embeddings = response.json()['predictions']  # List of embedding vectors

================================================================================
💡 BEST PRACTICES
================================================================================

1. **Batch Size**: Process 5-50 records per request for optimal performance
2. **Column Names**: Model auto-detects single columns, specify for multiple
3. **Error Handling**: Always check response.status_code before parsing
4. **Async Processing**: Use async requests for large batch operations
